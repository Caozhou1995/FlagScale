...
from megatron.core.models.multimodal.llavaov_model import LLaVAModel
...
def get_batch(data_iterator):
    ...
    input_ids_shape = tensor_parallel.broadcast_data(["input_ids_shape"], data, torch.int64)["input_ids_shape"]
    labels = tensor_parallel.broadcast_data(["labels"], data, torch.int64)["labels"]
    labels_shape = tensor_parallel.broadcast_data(["labels_shape"], data, torch.int64)["labels_shape"]
    images = tensor_parallel.broadcast_data(["images"], data, torch.float32)["images"]
    split_image_sizes = tensor_parallel.broadcast_data(["split_image_sizes"], data, torch.int64)["split_image_sizes"]
...
def forward_step(data_iterator, model: LLaVAModel):
    ...
    output_tensor, loss_mask = model(input_ids, labels, attention_mask, images, image_sizes, modalities)
...
def add_multimodal_extra_args(parser):
    ...
    group.add_argument("--pos-skipping-range",type=int, default=4096, help="Position skipping range")
    return parser
