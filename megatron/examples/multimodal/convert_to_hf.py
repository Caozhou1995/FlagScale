...
def convert(input_path, output_path, use_te, tensor_parallel_size=2):
    ...
    mc_model_0 = torch.load(os.path.join(input_path, "mp_rank_00", "model_optim_rng.pt"), map_location='cuda:0')
    mc_model_1 = torch.load(os.path.join(input_path, "mp_rank_01", "model_optim_rng.pt"), map_location='cuda:0')
    mc_model = {}
    llm_chunk_dim_0 = ["embedding.word_embeddings.weight", "self_attention.linear_qkv.weight", "self_attention.linear_qkv.bias", "mlp.linear_fc1.weight"]
    llm_chunk_dim_1 = ["self_attention.linear_proj.weight", "mlp.linear_fc2.weight"]
    vision_chunk_dim_0 = ["self_attention.linear_qkv.weight", "self_attention.linear_qkv.bias", "mlp.linear_fc1.weight", "mlp.linear_fc1.bias"]
    ...
    for name, param in mc_model_0["model"].items():
        ...
        if use_te and "_extra_state" in name:
            continue
        if is_chunked(name, llm_chunk_dim_0) or is_chunked(name, vision_chunk_dim_0) or is_chunked(name, mlp_chunk_dim_0):
            print(f"{name} concat in dim 0")
            mc_model[name] = torch.cat([param, mc_model_1["model"][name]], dim=0)
        elif is_chunked(name, llm_chunk_dim_1) or is_chunked(name, vision_chunk_dim_1) or is_chunked(name, mlp_chunk_dim_1):
            print(f"{name} concat in dim 1")
            mc_model[name] = torch.cat([param, mc_model_1["model"][name]], dim=1)
        else:
    ...
    for i in range(tensor_parallel_size):
        ...
        gate_up_indices.append(torch.arange(start + offset, start + offset + ffn_hidden_size // tensor_parallel_size, dtype=torch.int))
    ...
    for i in range(tensor_parallel_size):
        ...
        gate_up_indices.append(
            torch.arange(start + offset, start + offset + ffn_hidden_size // tensor_parallel_size, dtype=torch.int)
        )
    gate_up_indices = torch.cat(gate_up_indices)
    deorder_gate_up_indices = gate_up_indices

    input_layer_norm_weight = "input_layernorm.weight" if not use_te else "self_attention.linear_qkv.layer_norm_weight"
    input_layer_norm_bias = "input_layernorm.bias" if not use_te else "self_attention.linear_qkv.layer_norm_bias"

    post_attention_layer_norm_weight = "pre_mlp_layernorm.weight" if not use_te else "mlp.linear_fc1.layer_norm_weight"


    layer_norm_2_weight = "pre_mlp_layernorm.weight" if not use_te else "mlp.linear_fc1.layer_norm_weight"
    ...
    for mc_name in mc_model:
        ...
        if "language_model.embedding.word_embeddings.weight" in mc_name:
            ...
            if ("self_attention.linear_qkv.weight" in mc_name):
                ...
                qkv_weight = torch.split(mc_tensor, [hidden_dim, kv_projection_size, kv_projection_size])
                ...
                # deorder_indices
                mc_tensor = mc_tensor[deorder_indices]
                qkv_bias = torch.split(mc_tensor, [hidden_dim, kv_projection_size, kv_projection_size])
    ...
    hf_model["model.mm_projector.0.weight"] = mc_model["vision_projection.encoder.linear_fc1.weight"]
    hf_model["model.mm_projector.0.bias"] = mc_model["vision_projection.encoder.linear_fc1.bias"]
    hf_model["model.mm_projector.2.weight"] = mc_model["vision_projection.encoder.linear_fc2.weight"]
...
if __name__ == "__main__":
    convert("/share/project/caozhou/FlagScale/outputs/checkpoints/iter_0005208", 
                "/share/project/caozhou/deepspeed_stage_1.5_ckpt/", 
                True)
    ...
    # new_state_dicts = [{"model": dict()} for _ in range(1)]
