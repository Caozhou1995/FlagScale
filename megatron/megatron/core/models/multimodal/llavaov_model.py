...
class LLaVAModel(MegatronModule):
    ...
    def __init__(
        ...
        if "unpad" in args.mm_patch_merge_type:
            ...
            self.image_newline = torch.nn.Parameter(torch.randn(args.hidden_size, dtype=torch.bfloat16) * embed_std)
    ...
    def encode_images(self, images):
        ...
        encoded_image_features = encoded_image_features.permute(
            1, 0, 2
        ).contiguous()
    ...
    def prepare_inputs_labels_for_multimodal(self, input_ids, position_ids, attention_mask, labels, images, modalities=["image"], image_sizes=None, past_key_values=None):
        ...
        if text_modality and image_or_video_modality:
            raise ValueError("Text and image/video modalities cannot be mixed in the same batch.")
                ...
                for image_idx, image_feature in enumerate(image_features):
                    if image_feature.shape[0] > 1:
                        ...
                        if image_aspect_ratio == "anyres" or "anyres_max" in image_aspect_ratio:
                            ...
                            assert args.image_grid_pinpoints is not None, "image_grid_pinpoints must be provided."
                            num_patch_width, num_patch_height = get_anyres_image_grid_shape(image_sizes[image_idx], args.image_grid_pinpoints, vision_tower_image_size)
    ...
    def embed_tokens(self, input_ids):
        language_embeddings = self.language_model.embedding(input_ids, position_ids=None) # [text_seq_len, b, h_language]
